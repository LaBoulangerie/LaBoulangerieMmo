...
public class BlockusRestoration extends BukkitRunnable {
    ...
    public void run() {
        ...
        BlockusDataHolder blockusDataHolder =
                LaBoulangerieMmo.PLUGIN.getBlockusDataManager().getBlockusDataHolder();

        for (Iterator<Blockus> iterator =
                blockusDataHolder.getBlockuses().values().iterator(); iterator.hasNext();) {
            ...
            try {
                ...
                LaBoulangerieMmo.PLUGIN.getLogger().warning(
                        "A world has been deleted so all its blockuses have been dropped.");
            }
        }
    }
}
