...
public class RendererProvider {
    ...
    public void store(int id, LeaderBoardRenderer renderer) {
        FileUtils.save(new File(dataFaloder, id + ".json"), LaBoulangerieMmo.PLUGIN.getSerializer().serialize(renderer));
    }

    public LeaderBoardRenderer provide(int id) {
        return (LeaderBoardRenderer) LaBoulangerieMmo.PLUGIN.getSerializer().deserialize(FileUtils.read(new File(dataFaloder, id + ".json")), LeaderBoardRenderer.class);
    }
