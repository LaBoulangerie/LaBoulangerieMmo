...
public class TalentsRegistry {
     ...
     * 
     * @param archetype
     * @return false if the {@link net.laboulangerie.laboulangeriemmo.api.talent.TalentArchetype TalentArchetype} is
     *         invalid (e.g: talent already exists or references a non-existent ability). Returns true otherwise
     */
    ...
    public void init() {
        ...
        YamlConfiguration config =
                YamlConfiguration.loadConfiguration(new File(LaBoulangerieMmo.PLUGIN.getDataFolder(), "config.yml"));
        ...
        if (config.getBoolean("enable-talents", false) && config.isSet("talents")) {
            ...
            for (String identifier : config.getConfigurationSection("talents").getKeys(false)) {
                try {
                    ...
                    ConfigurationSection talentConfig = config.getConfigurationSection("talents." + identifier);

                    // TODO : Build something like a ConfigUtils to parse types, Material...
                    // TODO : Break down portions of this method, it's too big
                    String talentDisplayName = talentConfig.getString("display_name");
                    if (talentDisplayName == null) {
                    ...
                    talent.displayName = talentDisplayName;

                    if (talentConfig.isSet("combo_items")) {
                        try {
                            talent.comboItems = talentConfig.getStringList("combo_items").stream()
                                    .map(Material::valueOf).collect(Collectors.toList());
                        } catch (Exception e) {
                            LaBoulangerieMmo.PLUGIN.getLogger()
                                    .warning("Unable to parse optional field 'combo_item of talent '" + identifier
                                            + "', ignoring the field!");
                        }
                    }

                    if (talentConfig.isSet("abilities")) {
                        ConfigurationSection abilities = talentConfig.getConfigurationSection("abilities");
                        /**
                        ...
                        for (String abilityId : abilities.getKeys(false)) {
                            ...
                            // Appends if the path lead to a list (talking by experience)
                            if (ability == null) {
                            ...
                            requiredFields.keySet().stream().forEach(fieldName -> {
                                ...
                                // Dynamically assign values to each field of the AbilityArchetype
                                try {
                                    ...
                                    if (fieldName.equals("display_item")) value = Material.valueOf((String) value);

                                    classField.set(abilityArchetype, value);
                                } catch (Exception e) {
                                    LaBoulangerieMmo.PLUGIN.getLogger()
                                            .severe("Error when trying to set field '" + fieldName + "' of ability '"
                                                    + abilityId + "' from talent '" + identifier
                            ...
                            if (ability.isSet("tiers")) {
                                try {
                                    ...
                                    LaBoulangerieMmo.PLUGIN.getLogger()
                                            .warning("Error when trying to set optional field 'tiers' for ability '"
                    ...
                    LaBoulangerieMmo.PLUGIN.getLogger()
                            .warning("Unable to load talent \"" + identifier + "\" from configuration!");
                    e.printStackTrace();
                }
            }
        } else {
    ...
    /**
     * Generates a Map containing an instance of {@link net.laboulangerie.laboulangeriemmo.api.talent.Talent Talent} for
     * each registered {@link net.laboulangerie.laboulangeriemmo.api.talent.TalentArchetype TalentArchetype}. Used
     * internally to update data hold by {@link net.laboulangerie.laboulangeriemmo.api.player.MmoPlayer MmoPlayer}
     * 
    ...
    private void warnAbility(String talent, String ability, String cause) {
        LaBoulangerieMmo.PLUGIN.getLogger()
                .warning("Unable to load ability '" + ability + "' of talent '" + talent + "': " + cause);
    }
}
